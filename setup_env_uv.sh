#!/bin/bash
# ============================================================================
# Environment Setup Script (UV-based)
# ============================================================================
# Usage:
#   chmod +x setup_env_uv.sh
#
#   # M1 Mac (API-based LLM/VLM)
#   ./setup_env_uv.sh --mac
#
#   # GPU Server (AQI Evaluation + PyTorch + Flash-Attention)
#   ./setup_env_uv.sh --gpu
# ============================================================================

set -e  # Exit on error

# Show usage if no flag provided
if [ -z "$1" ]; then
    echo "Error: No flag provided"
    echo ""
    echo "Usage:"
    echo "  ./setup_env_uv.sh --mac   # M1 Mac (API-based)"
    echo "  ./setup_env_uv.sh --gpu   # GPU Server (AQI + PyTorch)"
    exit 1
fi

# Detect OS
OS="$(uname -s)"

# ============================================================================
# Install UV if not available
# ============================================================================
install_uv() {
    if ! command -v uv &> /dev/null; then
        echo "Installing UV package manager..."
        curl -LsSf https://astral.sh/uv/install.sh | sh
        export PATH="$HOME/.local/bin:$PATH"
    fi
    echo "UV: $(uv --version)"
}

# ============================================================================
# Common setup (both --mac and --gpu)
# ============================================================================
setup_base() {
    echo "============================================"
    echo "Environment Setup (UV)"
    echo "============================================"
    echo "Detected OS: $OS"
    echo ""

    # Create virtual environment (UV auto-downloads Python 3.12 if missing)
    if [ ! -d "venv_Agnt_Algnmt" ]; then
        echo "Creating virtual environment..."
        uv venv --python 3.12 venv_Agnt_Algnmt
    else
        echo "Virtual environment already exists"
    fi

    # Activate virtual environment
    source venv_Agnt_Algnmt/bin/activate

    # Install base requirements
    echo "Installing base requirements..."
    uv pip install -r requirements.txt

    echo ""
    echo "Base setup complete."
    echo ""
}

# ============================================================================
# --mac: M1 Mac setup (API-based)
# ============================================================================
if [ "$1" = "--mac" ]; then
    install_uv
    setup_base

    echo "============================================"
    echo "M1 Mac Setup Complete!"
    echo "============================================"
    echo ""
    echo "To activate environment:"
    echo "  source venv_Agnt_Algnmt/bin/activate"
    echo ""
    echo "Run tests:"
    echo "  python src/utils/config.py"
    echo "  python src/m02_measure_baseline_aqi.py --help"
    echo ""
    exit 0
fi

# ============================================================================
# --gpu: GPU Server setup (AQI Evaluation + PyTorch + Flash-Attention)
# ============================================================================
if [ "$1" = "--gpu" ]; then
    if [ "$OS" != "Linux" ]; then
        echo "Error: --gpu requires Linux. Detected: $OS"
        exit 1
    fi

    echo "============================================"
    echo "GPU Setup (Linux + Nvidia)"
    echo "============================================"

    # 0. Install system dependencies
    echo ""
    echo "[0/7] Installing system dependencies..."
    sudo apt update && sudo apt install -y \
        texlive-latex-base \
        texlive-latex-extra \
        texlive-fonts-recommended \
        texlive-fonts-extra \
        texlive-bibtex-extra \
        texlive-science \
        biber \
        tree \
        aria2

    # 1. Install UV (handles Python 3.12 automatically)
    echo ""
    echo "[1/7] Installing UV..."
    install_uv

    # Now create venv + install base requirements
    setup_base

    # 2. Install PyTorch 2.5.1 with CUDA 12.4
    echo ""
    echo "[2/7] Installing PyTorch 2.5.1+cu124..."
    uv pip install torch==2.5.1 torchvision==0.20.1 --index-url https://download.pytorch.org/whl/cu124

    # 3. Verify PyTorch
    echo ""
    echo "[3/7] Verifying PyTorch..."
    python -c "import torch; print(f'PyTorch: {torch.__version__}, CUDA: {torch.version.cuda}, Available: {torch.cuda.is_available()}')"

    # 4. Install GPU requirements
    echo ""
    echo "[4/7] Installing GPU requirements..."
    uv pip install -r requirements_gpu.txt --constraint requirements.txt

    # 5. Uninstall torchao (incompatible with torch 2.5.1 + transformers)
    echo ""
    echo "[5/7] Removing incompatible torchao..."
    uv pip uninstall torchao 2>/dev/null || true

    # 6. Install Flash-Attention
    echo ""
    echo "[6/7] Installing Flash-Attention 2.8.3..."
    WHEEL_NAME="flash_attn-2.8.3+cu12torch2.5cxx11abiFALSE-cp312-cp312-linux_x86_64.whl"
    WHEEL_URL="https://github.com/Dao-AILab/flash-attention/releases/download/v2.8.3/flash_attn-2.8.3%2Bcu12torch2.5cxx11abiFALSE-cp312-cp312-linux_x86_64.whl"

    # Clean any existing/partial wheel files before download
    rm -f flash_attn*.whl
    # Use aria2 with 16 parallel connections to bypass GitHub CDN throttling
    aria2c -x 16 -s 16 -o "$WHEEL_NAME" "$WHEEL_URL"
    uv pip install "$WHEEL_NAME"
    rm -f "$WHEEL_NAME"

    # 7. Final verification
    echo ""
    echo "[7/7] Verifying GPU setup..."
    python -c "
import torch
import transformers
print(f'PyTorch:      {torch.__version__}')
print(f'CUDA:         {torch.version.cuda}')
print(f'GPU:          {torch.cuda.is_available()}')
print(f'Transformers: {transformers.__version__}')
"

    echo ""
    echo "============================================"
    echo "GPU Setup Complete!"
    echo "============================================"
    echo ""
    echo "To run Phase 1 AQI evaluation:"
    echo "  python src/m02_measure_baseline_aqi.py --mode sanity"
    echo ""
    exit 0
fi

# Unknown flag
echo "Error: Unknown flag '$1'"
echo ""
echo "Usage:"
echo "  ./setup_env_uv.sh --mac   # M1 Mac (API-based)"
echo "  ./setup_env_uv.sh --gpu   # GPU Server (AQI + PyTorch)"
exit 1
